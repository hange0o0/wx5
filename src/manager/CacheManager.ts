class CacheManager_wx5{

    private static _instance:CacheManager_wx5;
    public static getInstance():CacheManager_wx5 {
        if (!this._instance)
            this._instance = new CacheManager_wx5();
        return this._instance;
    }
	private wx5_functionX_28085(){console.log(5323)}
    public registerData = {};
    public table = {};

    private cacheLoad = {};

    public constructor() {


    }
	private wx5_functionX_28086(){console.log(6440)}

    private register_3395(cls)
    {
        this.registerData[cls.dataKey] = cls;
    }

    //初始化数据
	private wx5_functionX_28087(){console.log(3294)}
    public initData(data,key){
        if(!this.table[key])
            this.table[key] = {};
        data = data.replace(/\r/g,'')
        var rows = data.split('\n')
        var fieldDelim = '\t';
	wx5_function(5278);
        var fields: Array<string> = String(rows[0]).split(fieldDelim);
        for(var i: number = 1;i < rows.length;i++) {
            var s: string = rows[i];
            if(s != null && s != "") {
                var cols: Array<any> = s.split(fieldDelim);
                var cls = this.registerData[key];
	wx5_function(9173);
                var vo:any = new cls();
                for(var j: number = 0;j < fields.length;j++) {
                    var value = cols[j];
                    vo[fields[j]] = value && !isNaN(value) ? Number(value) : value;
                }
                vo.reInit();
	wx5_function(1533);
                this.table[key][vo[cls.key]] = vo;
            }
        }
    }

    //静态数据初始化后调用
    public initFinish(){

    }
	private wx5_functionX_28088(){console.log(3681)}


}


//var a;
//var arr1 = [];
//for(var s in a)
//{
//    if(typeof a[s] == 'number')
//        arr1.push('public ' + s + ': number;')
//    else
//        arr1.push('public ' + s + ': string;')
//
//}
//for(var s in a)
//{
//    arr1.push('this.' + s + ' = data.' + s)
//}
//console.log(arr1.join('\n'))
