class ShareTool {
     ...
     * 分享
     */
	private wx5_functionX_28147(){console.log(8713)}
    public static share(title, imgUrl,shareArgs,success?,mustSuccess?){
        ...
        GameManager_wx5.getInstance().onShowFun = function(){
            if(mustSuccess || !UM_wx5.shareFail)
            {
                success && success();
                return;
            }
            ...
            if(cd < 2500)
            {
                GameManager_wx5.getInstance().shareFailTime ++;
                MyWindow.ShowTips(failText)
                return;
            }

            if(GameManager_wx5.getInstance().shareFailTime < 3)
            {
                if(Math.random()*(GameManager_wx5.getInstance().shareFailTime+1) < 0.8)
                {
                    GameManager_wx5.getInstance().shareFailTime ++;
                    MyWindow.ShowTips(failText)
                    return;
            ...
            GameManager_wx5.getInstance().shareFailTime = 0;
            success && success();
        }
        ...
        wx.shareAppMessage({
            title:title,
            imageUrl:imgUrl,
            query:ObjectUtil_wx5.join(shareArgs),
            cancel:()=>{
                GameManager_wx5.getInstance().onShowFun = null;
            }
        })
	...
	private wx5_functionX_28148(){console.log(1924)}
	...
	private wx5_functionX_28149(){console.log(5761)}
